...
# The landing and main menu, providing accesses to three features and a exit access. 
  ...
  # History feature(display all the existing history record to allow user to select one of them to browse further.)
    ...
    if  @history.history.size>0 && history_array.size>0
        history_array.each {|e|options.push({name:"Record：#{e["Id"]}  Date: #{e["Date"]}\n\n", value: -> {read_history(e["Id"])}})}
    end
    ...
    def read_history(id)
        ...
        puts
        puts "----------Record-------------\n\n"
        puts "Play\'s name"+" "*(30-"Play\'s name".length) + "#{@history.history["Records"][id-1]["User_name"]}".colorize(:light_blue)
        puts "Collection Name"+" "*(30-"Collection Name".length) + "#{@history.history["Records"][id-1]["Quiz_collection_name"]}".colorize(:light_cyan)
        puts "Collection Type"+" "*(30-"Collection Type".length) + "#{@history.history["Records"][id-1]["Type"]}".colorize(:light_magenta)
        puts "Accuracy Rate"+" "*(30-"Accuracy Rate".length) + "#{@history.history["Records"][id-1]["Accuracy_rate"]}".colorize(:light_yellow)
        puts "Correct Count"+" "*(30-"Correct Count".length) + "#{@history.history["Records"][id-1]["Correct"]}".colorize(:light_blue)
        puts "Incorrect Count"+" "*(30-"Incorrect Count".length) + "#{@history.history["Records"][id-1]["Incorrect"]}".colorize(:light_cyan)
   ...
   # Receive the custom object and collection object to perform deletion process
    def delete_decision(custom,collection)
        ...
        options.push(
          ...
          option = @prompt.select("The collection will be delete permanently!!! Think carefully before you act.".colorize(:blue ).colorize( :background => :red), options, help: "(Select with pressing ↑/↓ arrow keys, and then pressing Enter)\n\n\n", show_help: :always) 
    end
    ...
    def test_loop(type,quiz_name,quiz,user_name,time)
        ...
        status=[]
            ...
            begin Timeout.timeout(time) do
                ...
                puts "You have get "+"#{status[1]}".colorize(:light_yellow)+ " correct answer(s) so far.    And the total score is"+" #{status[0]}.\n\n".colorize(:light_blue)
...
# helper for save the custom file
    def save_custom_file(custom)
         ...
         begin @custom.save_custom(custom)
